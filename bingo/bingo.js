...
function showCellModal(title, content) {
  ...
  window.umami?.track('Bingo View', {name: title});
...
function initializeScanner() {
  for (const free of freeCells) {
    processQRCode(free, true);
  }
...
function processQRCode(data, scanned = false) {
  ...
  if (cell) {
    if (!scanned) {
      ...
      window.umami?.track('Bingo Stamp - ' + name, {id});
    }
...
function checkBingo(row, col) {
  ...
  // 2026: only the two diagonals win
  ...
  // Check full board (takes precedence over a diagonal)
  if (boardState.every(row => row.every(cell => cell !== null))) {
    bingoData = boardState.flat().map(cell => cell.rowData).join('');
    console.log("Full Bingo!", bingoData);
    i = 10;
  }
